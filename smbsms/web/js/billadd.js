...
$(function(){
	...
	billCode.on("blur",function(){
		if(billCode.val() != null && billCode.val() != ""){
			$.ajax({
				type:"GET",//请求类型
				url:path+"/billCode/"+billCode.val(),//请求的url
				dataType:"json",//ajax接口（请求url）返回的数据类型
				success:function(data) {//data：返回数据（json对象）
					if (data.billCode == "true"){
						validateTip(billCode.next(),{"color":"red"},imgNo+" 该编码已被使用，请重新输入",false);
					}else {
	...
	productDesc.on("focus",function(){
		validateTip(productDesc.next(),{"color":"#666666"},"* 请输入商品类型",false);
	}).on("blur",function(){
