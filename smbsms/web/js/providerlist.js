	...
	$.ajax({
		...
		success:function(data){
			if (data.delResult == "notexist") {
				...
				changeDLGContent("对不起，该供应商【"+obj.attr("proname")+"】下有【"+data.size+"】条订单，不能删除");
			}else if(data.delResult == "true"){//删除成功：移除删除行
				cancleBtn();
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/toProviderView?proid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/toProviderUpdate?proid="+ obj.attr("proid");
	});
