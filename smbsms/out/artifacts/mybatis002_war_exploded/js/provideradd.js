...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/pCode",//请求的url
		data:{proCode:proCode.val()},//请求参数
		dataType:"json",//ajax接口（请求url）返回的数据类型
		success:function(data){//data：返回数据（json对象）
			if(data.proCode == "error"){//Code输入为空
				validateTip(proCode.next(),{"color":"red"},imgNo + "Code不能为空，请重新输入",false);
			}else if(data.proCode == "false"){//Code已存在，错误提示
				validateTip(proCode.next(),{"color":"red"},imgNo+ " 该用Code已存在",false);
			}else if (data.proCode == "true") {//Code可用，正确提示
				validateTip(proCode.next(),{"color":"green"},imgYes+" 该Code可以使用",true);
			}
		},
	...
	*/
	...
	proPhone.on("focus",function(){
		...
		var patrn=/^(1[3-6][0-9]|15[0-9]|18[0-9])\d{8}$/;
