...
function deleteBill(obj){
	...
	$.ajax({
		type:"GET",
		url:path+"/deleteBill/"+obj.attr("billid"),
		dataType:"json",
		success:function(data){
			...
			} else if(data.delResult == "isNo"){//删除失败
				//alert("对不起，删除订单【"+obj.attr("billcc")+"】失败");
				changeDLGContent("对不起，订单【"+obj.attr("billcc")+"】还未付款,删除失败");
...
$(function(){
	...
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/toBillUpdate/"+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
