		...
		success:function(data){//data：返回数据（json对象）
			if(data != null){
				...
				for(var i = 0; i < rlist.length; i++){
					//alert(rlist[i].id);
					//alert(rlist[i].roleName);
					if(rid != null && rid != undefined && rlist[i].id == rid ){
						options += "<option selected=\"selected\" value=\""+rlist[i].id+"\" >"+rlist[i].roleName+"</option>";
					}else{
						options += "<option value=\""+rlist[i].id+"\" >"+rlist[i].roleName+"</option>";
					}
